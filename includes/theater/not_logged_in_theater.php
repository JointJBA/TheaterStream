...
$video_path = mysql_fetch_row(mysql_query("SELECT `video_path` FROM `users` WHERE `theater_name`='$theater_name'")); 
$video_path = $video_path[0]; // mysql_fetch_row returns an array. we only want the Name so we just set it excluseively. 

$video_type = mysql_fetch_row(mysql_query("SELECT `video_type` FROM `users` WHERE `theater_name`='$theater_name'")); 
$video_type = $video_type[0]; // mysql_fetch_row returns an array. we only want the Name so we just set it excluseively. 
...
var isadmin = false; //is it a client or an administrator?
...
function client() //updates the client
{
    $.get(getlink + "videoid=" + videoId, function(data)
    {
        ...
        if(Math.abs(player.playbackTime() - nlrt) > 1)
        {
            player.seekTo(nlrt);
        }
